...
class RunIB:
    ...
    #prior_spec = ["goal", "place", "source"]):
        ...
        self.prior = get_exp_prior(self.distal_levels, prior_spec, prior_source)  # p(x)
        self.pgs_dists = pgs_dists
        self.merge_d1d2 = merge_d1d2
            ...
            for distal_num in range(1, self.distal_levels + 1):
                for pgs in ["place", "goal", "source"]:
                    ...
                    if curkey not in unique_word_d:
                        ...
                        #print(f'language={lang}, prevkey={prevkey}, unique_word_d={unique_word_d}')
                        if self.merge_d1d2 and distal_num >2:
                            prev2key = "D{}_{}".format(str(distal_num - 2), pgs)
                            unique_word_d[prevkey] = unique_word_d[prev2key]
                            #print(f'language={lang}, prev2key={prev2key}, unique_word_d={unique_word_d}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--merge_d1d2', action='store_true')
    parser.add_argument('--prior_source', type = str, default = "lexiteria", help = "The source of the frequency distribution")
    ...
    #                                         outfile=args.outfile)
        ...
        for mu in np.append(np.linspace(0.05, 0.99, 19), np.array(0.99)):
            RunIB(mu, args.distal, args.pgs, prior_spec =["place", "goal", "source"] ).get_mi_for_all(
        ...
        RunIB(args.mu, args.distal, args.pgs, prior_source=args.prior_source, merge_d1d2=args.merge_d1d2).get_mi_for_all(sim_lex_dict=sim_lex_dict,
            outfile = args.outfile)